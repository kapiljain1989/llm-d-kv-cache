...
func TestHMAModelE2E(t *testing.T) {
	...
	populateIndex(t, indexer.KVBlockIndex(), map[kvblock.BlockHash][]kvblock.PodEntry{
		100: {
			{PodIdentifier: testPodA, DeviceTier: "gpu", StoredGroups: []int{0, 1}},
			{PodIdentifier: testPodB, DeviceTier: "gpu", StoredGroups: []int{0, 1}},
		},
		101: {
			{PodIdentifier: testPodA, DeviceTier: "gpu", StoredGroups: []int{0, 1}},
			{PodIdentifier: testPodB, DeviceTier: "gpu", StoredGroups: []int{0, 1}},
			{PodIdentifier: "pod-c", DeviceTier: "gpu", StoredGroups: []int{1}},
		},
		102: {
			{PodIdentifier: testPodA, DeviceTier: "gpu", StoredGroups: []int{0, 1}},
			{PodIdentifier: testPodB, DeviceTier: "gpu", StoredGroups: []int{0}},
			{PodIdentifier: "pod-c", DeviceTier: "gpu", StoredGroups: []int{1}},
		},
	})
	...
	// Verify scores with magnitude separation:
	// - Full attention blocks are multiplied by 100,000
	// - Sliding window blocks are added as raw scores
	// Expected scores:
	// - podA: full(3 blocks) * 100000 + swa(2 blocks, last 2) = 300,002
	// - podB: full(3 blocks) * 100000 + swa(0 blocks, chain breaks at 102) = 300,000
	// - podC: full(0 blocks) * 100000 + swa(2 blocks, last 2) = 2
	expectedScores := map[string]float64{
		testPodA: 300002.0, // 3 full + 2 swa
		testPodB: 300000.0, // 3 full + 0 swa (chain breaks)
		"pod-c":  2.0,      // 0 full + 2 swa
	}
	...
	// Verify full attention dominates:
	// podB (full attention only) should score higher than podC (sliding window only)
	assert.Greater(t, scores[testPodB], scores["pod-c"],
		"full attention should dominate sliding window due to magnitude separation")

	// Verify sliding window acts as tiebreaker:
	// podA (full + swa) should score higher than podB (full only)
	assert.Greater(t, scores[testPodA], scores[testPodB],
		"sliding window should act as tiebreaker when full attention scores are equal")
}
